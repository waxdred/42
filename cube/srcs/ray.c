...
static void	ray_horizontal(t_cube *cube, t_player *p, t_ray *ray)
{
	...
	while (ray->dof < 5)
	{
		ray->mx = (int)(ray->rx) >> 6;
		ray->my = (int)(ray->ry) >> 6;
		if (ray->mp > 0 && ray->mp < cube->i_map->mapX * cube->i_map->mapY && cube->map[ray->my][ray->mx] == '1')
		{
			ray->hx = ray->rx;
			ray->hy = ray->ry;
...
void	draw_ray_3d(t_cube *cube, t_player *p)
{
	...
	while ( ray.r < DOF)
	{
		...
		ray.lineH = (SQUARES * cube->wd_height) / ray.disT;
		if (ray.lineH > cube->wd_width)
			ray.lineH = cube->wd_width;
		ray.lineO = cube->wd_height / 2 - (ray.lineH / 2);
		int	i;
		int	size;

		size = (int)(cube->wd_width / DOF) + 1;
		i = 0;
		while (i++ < size)
		{
			/* ft_start_draw(cube->line, ray.r * size + size - 1, ray.lineO, color); */
			/* ft_end_draw(cube->line, ray.r * size * size - 1, ray.lineH + ray.lineO); */
